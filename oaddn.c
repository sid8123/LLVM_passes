...
//==================== Manually modified code ===============================//  
  //Below code is expansion of cnot and sigma_x functions and possible fusion
  if (!(compare & (1<<(width-1)))) {
  
    quantum_qec_get_status(&qec, NULL);
    if(qec){
        quantum_sigma_x_ft(2*width-1, reg);
        quantum_cnot_ft(2*width-1, width-1, reg);
    }
  else {
    int V1 = quantum_objcode_put(SIGMA_X, 2*width-1);
    int V2 = quantum_objcode_put(CNOT, 2*width-1, width-1);
    if(V1 || V2)
      return;
    ...
    for(i=0; i<reg->size; i++)
      {
        ...
        reg->node[i].state ^= ((MAX_UNSIGNED) 1 << 2*width-1);
        if((reg->node[i].state & ((MAX_UNSIGNED) 1 << 2*width-1)))
          reg->node[i].state ^= ((MAX_UNSIGNED) 1 << width-1);   
